...
# This script also requires bioperl. 
...
external dependencies: BioPerl, blastall\/makeblastdb (>2.2.17), exonerate, cd-hit-est, framedp 
...
sub recipBlast {
	...
	system ("sort -k 2,2 -k 12n,12r -k 11n,11r $recip > $recip1");
	open(IN, "<$recip1");
	while(<IN>) {
		chomp(my $line = $_);
		my @d = split(/\t/,$line);
