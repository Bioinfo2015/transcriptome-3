...
sub usage {
  die(qq/
Usage: 3-generateAssemblies_LOCAL.pl <command> [<arguments>]\n
Command: 

trinity: Assembly using trinity

abyss: Assembly using ABySS (build with MPI support)

soapdenovo: Assembly using soapdenovo (to be implemented, but not recommended for exon capture assembly)
...
sub abyss {
  die(qq/
Usage 3-generateAssemblies_LOCAL.pl abyss [options]

Options: -reads    CHAR            Directory with all sequence reads
         -lib      CHAR ...        Particular libraries to process? (e.g. AAA BBB CCC). If -lib is not used then process all libraries in the folder (-reads) 
         -mpi      CHAR            full path to openmpi mpirun   
         -out      CHAR            Directory where results will go
         -kmer     INT ...         A list of kmers. At least privide one kmer (e.g. 21 43 67 81)
         -kcov     INT ...         A list of non-zero kmer-coverage. Activate default without using kcov (e.g. 5 10 20). Suggest use default.
         -np       INT             number of processors used for assembly
         

reads are named as AAA_1_final.txt AAA_2_final.txt and AAA_u_final.txt 
Assume you will use Abyss for exon capture data. Multi-kmer assembly method (Singhal S. 2012. Molecular Ecology Resources; Bi et al. 2012. BMC genomics)
